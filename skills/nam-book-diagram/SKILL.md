...
Read the approved diagram request, project and locale profiles, objective map, source passage, evidence ledger, visual brief, project `illustration_setup` and `character_bible`, caption plan, and final placed dimensions. Preserve all `figure_id`, `objective_id`, `claim_id`, and `source_id` values. AskNam must have a current `illustration-setup` approval bound to both setup artifacts, including for diagram-only and mascot-free projects. If it is missing or stale, return a setup blocker for `nam-book-illustration-setup` instead of choosing a new style locally.
...
4. Follow [references/svg-production.md](references/svg-production.md). Apply the approved hand-drawn contour, shape, texture, and palette decisions without distorting exact relationships or text. A mascot appears only when the approved brief requests one and follows the selected project bible; Nam is not a universal diagram decoration. Start from `assets/diagram-template.svg` when useful, while replacing all example content and accessibility text.
...
Write `diagram_spec` and editable `diagram_asset` artifacts to the declared registry paths using the canonical artifact envelope. Record source and input hashes, including the setup and bible revisions; objective/claim/source mappings; locale; viewBox; intended placed size; font and color decisions; caption; alt text or long description; validation results; provenance; status; and blockers. Do not mutate global state or approve the diagram on behalf of a content expert.
