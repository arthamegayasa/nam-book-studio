...
| Illustration setup | Completed setup stage; current human `illustration-setup` approval names fresh `illustration_setup` and `character_bible` revisions; reference roles distinguish identity from style; calibration and source-asset hashes match; images retain approved hand-drawn tokens | When illustrations or diagrams are enabled, including mascot-free projects |
| Character and culture | Images preserve the selected project's silhouette and fixed anchors; custom or supplied mascots follow their own bible; selected Nam preserves its approved round charcoal eyes and no blue eye markings; `mascot_mode: none` adds no mascot; cultural references have sources, purpose, and review where needed | When a mascot or cultural material appears; also check explicit mascot absence for `none` |
